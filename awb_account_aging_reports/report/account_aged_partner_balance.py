...
class ReportAgedPartnerBalance(models.AbstractModel):
    ...
    def _get_partner_move_lines(self, account_type, date_from, target_move, period_length):
        ...
        analytic_query = ''
        if ctx.get('analytic_accounts', []):
            analytic_accounts = ",".join([str(id) for id in ctx.get("analytic_accounts")])
            analytic_query += f' AND l.analytic_account_id IN ({analytic_accounts})'

        if ctx.get('analytic_tags', []):
            analytic_tags = ",".join([str(id) for id in ctx.get("analytic_tags")])
            analytic_query += f''' AND l.id IN (
        ...
        for i in range(5):
            ...
            cr.sql_log = True
            _logger.debug(f'Query: {query} Args: {args_list}')
        ...
        for partner in partners:
            ...
            if at_least_one_amount or (self._context.get('include_nullified_amount') and partner['partner_id'] and lines[partner['partner_id']]):
                res.append(values)
